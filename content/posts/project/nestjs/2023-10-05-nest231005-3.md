...
[링크](http://localhost:3000) 접속해봐도 hi there!라고 잘뜬다.
...
`npx ts-node-dev src/main.ts`를 터미널에서 실행해보면 [링크](http://localhost:3000)가 여전히 잘 실행된다!

## Routing Decorators

컨트롤러 클래스에서 @get() decorator에 '/asdf' 를 인자로 넣어주면 [링크](http://localhost:3000) 에 오류가 뜬다.
...
대신 [/asdf를 추가한 링크](http://localhost:3000/asdf) 로 가면 이전처럼 'hi there!'이 잘뜬다.

@Controller decorator에도 적용할 수 있다. '/app'을 추가하여 [새로운 링크](http://localhost:3000/app/asdf) 로 들어가야하면 'hi there!'를 만날 수 있다.
...
[들어가서 잘되나 보자,](http://localhost:3000/app/bye)
