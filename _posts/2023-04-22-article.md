...
priority inheritance protocol의 기본 아이디어는 한 작업이 더 높은 우선순위의 작업에게 가로막혔을 때, 그 작업이 원래의 우선순위를 무시하고 상승한 우선순위 수준에서 critical section을 수행하는 것이다. critical section을 수행하고 lock을 release하고 나면, 포로세스는 원래의 우선순위 수준으로 돌아간다.
