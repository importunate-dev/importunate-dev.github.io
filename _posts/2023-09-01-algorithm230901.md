...
여전히 위상정렬 문제이다. 연결된 간선의 출발 정점과 도착 정점의 번호는 도착 정점의 번호가 더 커야한다고 한다. 따라서 출발정점은 n번이 될 수 없다. n보다 큰 정점은 없기 때문이다. 따라서 진입차수가 0인 정점이 가장 큰 값을 지녀야 한다. 그 이후 진입차수 내림차순으로 새로운 정점 번호를 고쳐주면 된다. 

만약 진입차수가 0인 정점이 2개라면? 둘 중 숫자가 더 큰 정점을 먼저 처리해야 한다. 이것을 위해서는 최대힙을 사용하였다. 문제가 원하는 바를 이해하기 든 문제긴 했지만 요구하는 것을 이해한다면 생각보다 쉽게 풀리는 문제였다.
